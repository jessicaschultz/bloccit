...
#Specs for guest users, who we will redirect to sign in if they attempt to create or delete a comment.
  context "guest" do
...
#specs for member users who attempt to create new comments or delete comments they don't own. We will allow member users to create new comments, but we'll redirect them to the posts show view if they try to delete another user's comment.
...
#code from checkpoint fails all test with ArgumentError: unknown keywords: post_id, id referencing the line in each test that has 'post :create, post_id: my_post.id, comment: {body: RandomData.random_sentence}'.
##in order to make test pass I have to wrap post_id etc. in params: {}
